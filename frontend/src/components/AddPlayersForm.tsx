...
const AddPlayersForm: React.FC<AddPlayersFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        const playersAdded = response.players?.length || players.length;
        Alert.alert(
          'Success!',
          response.message || `${playersAdded} player(s) added successfully. Welcome emails have been sent.`,
          [
            {
              text: 'OK',
              onPress: () => onSuccess(playersAdded),
            },
          ]
        );
      } else {
        Alert.alert('Error', response.message || 'Failed to add players');
      }
    } catch (error: any) {
      ...
      Alert.alert(
        'Error',
        error.response?.data?.error || error.message || 'Failed to add players. Please try again.'
      );
    } finally {
      setLoading(false);
    }
